...
for m in $(ls $savpath/machines.d | grep conf$); do
  ...
  /usr/local/admin/sysutils/zfs/zfs_clean_snap -r 72h15d6w12m1y $SAVZFSBASE/$NAME
  zfs list -H -o name -r -t snapshot $SAVZFSBASE/$NAME | fgrep '@'$NAME'-' | fgrep -v '@'$NAME'-'$(hostname -s) | xargs -tL1 zfs destroy -d
