...
for m in $machines; do
  ...
  if ! ssh -oConnectTimeout=2 -oPasswordAuthentication=no -oStrictHostKeyChecking=yes -oIdentitiesOnly=yes -oIdentityFile=$SSH_KEY -oKbdInteractiveDevices=none root@$DEST "echo is ok." || [ -n "$FORCE" ]; then
    MYIP4=$(ssh -4 -oConnectTimeout=2 -oPasswordAuthentication=no -oStrictHostKeyChecking=yes -oIdentitiesOnly=yes -oIdentityFile=$SSH_KEY -oKbdInteractiveDevices=none root@$DEST "echo \$SSH_CLIENT | awk '{print \$1}'")
    MYIP6=$(ssh -6 -oConnectTimeout=2 -oPasswordAuthentication=no -oStrictHostKeyChecking=yes -oIdentitiesOnly=yes -oIdentityFile=$SSH_KEY -oKbdInteractiveDevices=none root@$DEST "echo \$SSH_CLIENT | awk '{print \$1}'")
    ssh-keygen -R $DEST
    if [ -z "$MYIP6" ]; then
      MYIP=$MYIP4
    elif [ -z "$MYIP4" ]; then
